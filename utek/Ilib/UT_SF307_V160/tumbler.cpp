...
void Tumbler::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if (pressed) {
        ...
        if (oldIndex != index) {
            emit currentIndexChanged(index, oldIndex);
            emit currentValueChanged(listValue.at(index), oldIndex < 0 ? "" :listValue.at(oldIndex));
            oldIndex = index;
        }
...
void Tumbler::paintEvent(QPaintEvent *)
{
    ...
    if(selectIndex == index){
        drawSelectText(&painter, offset);
    }else if(selectIndex == index - 1){
        drawSelectText(&painter, offset - target / percent);
    }else if(selectIndex == index + 1){
        drawSelectText(&painter, offset + target / percent);
    }else{
        ;
...
void Tumbler::drawText(QPainter *painter, int index, int offset)
{
    ...
    font.setPixelSize((target - qAbs(offset)) / 11);
...
void Tumbler::drawSelectText(QPainter *painter, int offset)
{
    painter->save();
    if(selectIndex != -1){
        painter->setPen(selectGround);
        this->drawText(painter, selectIndex, offset);
    }
    painter->restore();
}
...
void Tumbler::addValue(const QString value)
{
...
void Tumbler::setCurrentIndex(int currentIndex)
{
    if (currentIndex >= 0) {
        ...
        emit currentIndexChanged(this->currentIndex, oldIndex);
        emit currentValueChanged(this->currentValue, oldIndex < 0 ? "" :listValue.at(oldIndex));
        update();
    }
}
...
void Tumbler::setCurrentValue(const QString &currentValue)
{
    if (listValue.contains(currentValue)) {
        QString oldValue = this->currentValue;
        this->currentValue = currentValue;
        this->currentIndex = listValue.indexOf(currentValue);
        emit currentIndexChanged(this->currentIndex, listValue.indexOf(oldValue));
        emit currentValueChanged(this->currentValue, oldValue);
        update();
    }
}
void Tumbler::setSelectEdValue(const QString value)
{
    if (listValue.contains(value)) {
