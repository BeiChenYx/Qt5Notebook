...
void TextMoveWidget::setText(const QString &text)
{
    if (this->text != text) {
        this->text = text;
        labText->setText(this->text);

        //自动拉伸标签载体尺寸
        int newTextWidth = this->fontMetrics().width(text);
        int textWidth = fontMetrics().width(this->text);
        int textHeight = fontMetrics().height();
        labText->resize(QSize(textWidth + newTextWidth, textHeight + 5));
...
void TextMoveWidget::setPaused(bool paused)
{
    int textWidth = this->fontMetrics().width(text);
    if (moveStyle == MoveStyle_LeftToRight) {
        this->initX = 0;
    }else{
        this->initX = -textWidth;
    }
