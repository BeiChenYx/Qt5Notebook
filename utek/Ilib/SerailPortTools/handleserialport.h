...
public:
    ...
    */
    ...
    // 设置阻塞通信
    void setBlock(bool isBlock=false);

    // 阻塞方式写入数据，并等待应答，默认超时1000毫秒，读取超时或异常，返回空的QByteArray
    QByteArray reqAndResponseBlock(const QByteArray &bytes, int timeout=1000);
