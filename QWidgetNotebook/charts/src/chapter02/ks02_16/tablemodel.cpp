...
bool CTableModel::insertColumns(int column, int count, const QModelIndex &parent)
{
	if (column < 0)
		column = 0;
	beginInsertColumns(parent, column, column + count - 1);// 重新设置每行数据的大小
	if (0 != m_columnCount)
	{
		column++;
	}
    // 从第column列开始补充数据	
	for (int i = 0; i < m_rowCount; i++) {
		QVector<qreal>* dataVec = m_data[i];
		for (int k = 0; k < count; k++) {
			dataVec->insert(k+ column, 0);
			if (k % 2 == 0)
				dataVec->replace(k+ column, i * 50 + QRandomGenerator::global()->bounded(20));
			else
				dataVec->replace(k+ column, QRandomGenerator::global()->bounded(100 * ((k+column + 1) / 2)));
		}
	}
...
bool CTableModel::removeColumns(int column, int count, const QModelIndex &parent)
{
	if ((column < 0) || (0==m_columnCount))
		return false;
	beginRemoveColumns(parent, column, column + count - 1);
	for (int i = 0; i < m_rowCount; i++) {
		QVector<qreal>* dataVec = m_data[i];
		for (int j = column; j < (m_columnCount-count); j++) {
			dataVec->replace(j, dataVec->at(j+count));
		}
		dataVec->resize(m_columnCount-count);
	}
