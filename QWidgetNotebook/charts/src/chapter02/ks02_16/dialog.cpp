...
void CDialog::initializeCurveSeries()
{
	...
	// curve table
	m_curveTableView = new QTableView(tab);
	m_curveModel = new CTableModel;
	m_curveTableView->setModel(m_curveModel);

	m_pCurveChartView = new CChartView(m_curveChart, tab);
	m_pCurveChartView->setObjectName(QStringLiteral("m_pCurveChartView"));
	...
	QGridLayout* gridLaytoutBar = new QGridLayout(tab);
...
void CDialog::initializeBarSeries()
{
	...
	m_pBarChartView->setObjectName(QStringLiteral("BarChartView"));
...
{
	m_barModel->removeColumns(m_barModel->columnCount() - 1, 1);
...
void CDialog::updateUI()
{
	...
	m_pCurveChartView->setRenderHint(QPainter::Antialiasing, checked);
...
void CDialog::connectMarkers()
{
	...
	{
		sets = m_seriesBar->barSets();
		for (QLegendMarker *marker : markers) {
			m_map[marker] = sets.at(idx++);			
		}
	}
...
void CDialog::handleMarker(QLegendMarker* marker)
{
...
void CDialog::handleMarkerClicked()
{
	...
	case QLegendMarker::LegendMarkerTypeBar:
		{
			marker->setVisible(true);
			QMap<QLegendMarker*, QBarSet*>::iterator iteMap = m_map.find(marker);
			if (iteMap != m_map.end()) {
				m_seriesBar->remove(iteMap.value());
			}
