...
QRectF CTip::boundingRect() const
{
    ...
    QRectF rect;
    rect.setLeft(qMin(m_rctTip.left(), anchor.x()));
    rect.setRight(qMax(m_rctTip.right(), anchor.x()));
    rect.setTop(qMin(m_rctTip.top(), anchor.y()));
    rect.setBottom(qMax(m_rctTip.bottom(), anchor.y()));
    return rect;
}
