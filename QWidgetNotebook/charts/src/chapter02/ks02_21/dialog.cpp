...
void CDialog::initializePolar()
{
	const qreal angularMin = -50;
	const qreal angularMax = 50;
	...
	m_polarChart->setTitle("Polar Chart");

	// 构建雷达图的视图对象
	m_pPolarChartView = new QChartView(m_polarChart, tab_polar);
	m_pPolarChartView->setObjectName(QStringLiteral("PieChartView"));

	// 上限折线
	QLineSeries *seriesLineOutter = new QLineSeries();
	seriesLineOutter->setName("area outer line");
	qreal ad = (angularMax - angularMin) / 8;
	qreal rd = (radialMax - radialMin) / 3 * 1.3;
	seriesLineOutter->append(angularMin, radialMax);
	seriesLineOutter->append(angularMin + ad * 1, radialMin + rd);
	seriesLineOutter->append(angularMin + ad * 2, radialMax);
	seriesLineOutter->append(angularMin + ad * 3, radialMin + rd);
	seriesLineOutter->append(angularMin + ad * 4, radialMax);
	seriesLineOutter->append(angularMin + ad * 5, radialMin + rd);
	seriesLineOutter->append(angularMin + ad * 6, radialMax);
	seriesLineOutter->append(angularMin + ad * 7, radialMin + rd);
	seriesLineOutter->append(angularMin + ad * 8, radialMax);
	
	// 下限折线
	QLineSeries *seriesLineInner = new QLineSeries();
	seriesLineInner->setName("area inner line");
	ad = (angularMax - angularMin) / 8;
	rd = (radialMax - radialMin) / 3;
	seriesLineInner->append(angularMin, radialMax);
	seriesLineInner->append(angularMin + ad * 1, radialMin + rd);
	seriesLineInner->append(angularMin + ad * 2, radialMax);
	seriesLineInner->append(angularMin + ad * 3, radialMin + rd);
	seriesLineInner->append(angularMin + ad * 4, radialMax);
	seriesLineInner->append(angularMin + ad * 5, radialMin + rd);
	seriesLineInner->append(angularMin + ad * 6, radialMax);
	seriesLineInner->append(angularMin + ad * 7, radialMin + rd);
	seriesLineInner->append(angularMin + ad * 8, radialMax);

	// 面积系列
	QAreaSeries *seriesArea = new QAreaSeries();
	seriesArea->setName("area");
	seriesArea->setUpperSeries(seriesLineOutter);
	seriesArea->setLowerSeries(seriesLineInner);
	seriesArea->setOpacity(0.5);
	
	// 曲线系列
	QSplineSeries *seriesSpline = new QSplineSeries();
	seriesSpline->setName("spline");
	for (int i = angularMin; i <= angularMax; i += 10)
		seriesSpline->append(i, (i / radialMax) * radialMax);

	// 将系列添加到图表
	m_polarChart->addSeries(seriesSpline);
	m_polarChart->addSeries(seriesLineOutter);
	m_polarChart->addSeries(seriesLineInner);
	m_polarChart->addSeries(seriesArea);

	// 添加坐标轴
	QValueAxis *angularAxis = new QValueAxis();
	angularAxis->setTickCount(9); // 第一个刻度和最后一个刻度坐落在0/360°位置。
	angularAxis->setLabelFormat("%.1f");
	//angularAxis->setShadesVisible(true);
	angularAxis->setShadesBrush(QBrush(QColor(249, 249, 255)));
	m_polarChart->addAxis(angularAxis, 
					QPolarChart::PolarOrientationAngular);// 本坐标位于雷达图外延(外周)

	QValueAxis *radialAxis = new QValueAxis();
	radialAxis->setTickCount(9);
	radialAxis->setLabelFormat("%d");
	m_polarChart->addAxis(radialAxis, 
				QPolarChart::PolarOrientationRadial); // 本坐标位于雷达图半径方向

	// 将坐标轴绑定到各个系列
	seriesSpline->attachAxis(radialAxis);
	seriesSpline->attachAxis(angularAxis);
	seriesLineOutter->attachAxis(radialAxis);
	seriesLineOutter->attachAxis(angularAxis);
	seriesLineInner->attachAxis(radialAxis);
	seriesLineInner->attachAxis(angularAxis);
	...
	m_polarChart->setTitleBrush(Qt::NoBrush);
	seriesArea->setBrush(Qt::yellow);
	seriesArea->setOpacity(0.5);

	// 构建布局对象，将图表的视图添加到布局对象
	QGridLayout* gridLaytoutPolar = new QGridLayout(tab_polar);
	gridLaytoutPolar->addWidget(m_pPolarChartView, 0, 0);
	gridLaytoutPolar->setColumnStretch(0, 1);
...
void CDialog::tabChanged()
{
	...
	case 3:
	default:
		m_chart = m_polarChart;
		m_pChartView = m_pPolarChartView;
		break;
	}
	disconnectMarkers();
	connectMarkers();
...
void CDialog::updateUI()
{
	...
	if (rbPlotAreaBrush_1->isChecked()) {
		...
		m_chart->setPlotAreaBackgroundBrush(Qt::lightGray);
	}
