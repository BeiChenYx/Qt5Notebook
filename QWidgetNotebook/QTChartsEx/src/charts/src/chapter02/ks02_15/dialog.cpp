...
void CDialog::initialize()
{
	...
	//gridLayout_7->addWidget(tableView, 0, 0, 1, 1, Qt::AlignTop| Qt::AlignLeft);

	m_pChartView = new CChartView(m_chart, widget);
	m_pChartView->setObjectName(QStringLiteral("m_pChartView"));
	//gridLayout_7->addWidget(m_pChartView, 0, 1, 1, 1, Qt::AlignTop| Qt::AlignLeft);

	gridLayout_7->addWidget(tableView, 1, 0);
	gridLayout_7->addWidget(m_pChartView, 1, 1);
	gridLayout_7->setColumnStretch(1, 1);
	gridLayout_7->setColumnStretch(0, 0);
	...
	m_chart->addSeries(m_series1);	// 添加系列,该调用要在setModel()之后。
	...
	mapper->setModel(model);
	m_chart->addSeries(m_series2);	// 添加系列,该调用要在setModel()之后。
	...
	m_axisX->setRange(0, m_series1->points()[model->rowCount()-1].x());	// 设置坐标范围
	...
	m_axisY3->setLabelsPosition(QCategoryAxis::AxisLabelsPositionCenter);

	m_chart->addAxis(m_axisY3, Qt::AlignRight);
