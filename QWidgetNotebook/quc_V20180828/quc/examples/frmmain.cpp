...
void frmMain::addItem(const QString &parentName, const QStringList &list)
{
    int count = list.count();
    listItem.append(QString("%1||1|%2|").arg(parentName).arg(count));
    for (int i = 0; i < count; i++) {
        listItem.append(QString("%1|%2|1||0xf0da").arg(list.at(i)).arg(parentName));
    }
}

void frmMain::addGauge()
{
    listGauge << "汽车仪表盘" << "圆弧仪表盘" << "半圆仪表盘" << "进度仪表盘" << "范围仪表盘"
              << "速度仪表盘" << "钟形仪表盘" << "网络仪表盘" << "简单仪表盘" << "时钟仪表盘"
              << "指南针仪表盘" << "旋转仪表盘" << "迷你仪表盘" << "圆环仪表盘" << "飞机仪表盘"
              << "LPM仪表盘" << "多彩仪表盘";
...
{
    listImage << "广告轮播" << "动画图片按钮" << "设备防区" << "图片开关" << "图片浏览"
              << "图片背景日历" << "图片背景电池" << "图片背景时钟" << "图片指示灯";
    addItem("贴图控件", listImage);
}

void frmMain::addPainter()
{
    listPainter << "高亮按钮" << "发光按钮" << "魔法小鱼" << "魔法老鼠" << "魔法水滴"
                << "游动的鱼" << "旋转圆" << "多彩旋转" << "开关按钮" << "烤箱旋转按钮"
...
void frmMain::addOther()
{
    listOther << "移动控件" << "面板元素" << "面板窗体" << "LCD时间" << "LCD数码管"
              ...
              << "验证码" << "图标编辑器" << "IP地址输入框" << "FlatUI" << "自定义下拉框";
    addItem("其他控件", listOther);
}

void frmMain::addDemo()
{
    listDemo << "放大镜" << "图标拖曳" << "颜色曲线";
    addItem("内置控件", listDemo);
}

void frmMain::addHide()
{
    listHide << "屏幕截图" << "颜色拾取器" << "遮罩层" << "托盘图标" << "蚂蚁线";
    addItem("不可见控件", listHide);
}

void frmMain::pressed(const QString &text, const QString &parentText)
{
