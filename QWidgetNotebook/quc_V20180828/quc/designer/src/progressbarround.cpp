...
void ProgressBarRound::paintEvent(QPaintEvent *)
{
    ...
    p.fillRect(baseRect, Qt::transparent);
...
void ProgressBarRound::drawProgress(QPainter *painter, const QRectF &baseRect)
{
    if (value == minValue) {
        return;
    }
...
void ProgressBarRound::drawValue(QPainter *painter, const QRectF &innerRect, double innerRadius)
{
    if (format.isEmpty()) {
        return;
    }
...
void ProgressBarRound::setBarStyle(const ProgressBarRound::BarStyle &barStyle)
{
