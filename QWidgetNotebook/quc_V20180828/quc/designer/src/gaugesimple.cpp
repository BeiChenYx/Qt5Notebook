...
void GaugeSimple::drawArc(QPainter *painter)
{
    ...
    if (showUserValue) {
        angleCurrent = angleAll * ((userValue - minValue) / (maxValue - minValue));
        pen.setColor(userValueColor);
    }
...
void GaugeSimple::drawValue(QPainter *painter)
{
    int radius = 100;
    painter->save();
...
void GaugeSimple::drawTitle(QPainter *painter)
{
    ...
    QRectF titleRect(-radius, 30, radius * 2, radius - 30);
...
bool GaugeSimple::getShowUserValue() const
{
    return this->showUserValue;
}

double GaugeSimple::getUserValue() const
{
    return this->userValue;
}

QColor GaugeSimple::getUserValueColor() const
{
...
void GaugeSimple::setValue(int value)
{
...
void GaugeSimple::setShowUserValue(bool showUserValue)
{
...
void GaugeSimple::setUserValue(double userValue)
{
    //值小于最小值或者值大于最大值或者值和当前值一致则无需处理
...
void GaugeSimple::setUserValue(int userValue)
{
    setUserValue((double)userValue);
}

void GaugeSimple::setUserValueColor(const QColor &userValueColor)
{
...
void GaugeSimple::setPointerStyle(const GaugeSimple::PointerStyle &pointerStyle)
{
