...
void RulerScale::paintEvent(QPaintEvent *)
{
    ...
    double margin = qMax(fabs(minValue), fabs(maxValue));
    ...
    double radius = 0.125 * pow( wScale, 2) / hScale + 0.5 * hScale;

    //计算中心点坐标
    QPointF center;
    if(radius < hScale + 0.5 * hWidget - borderWidth) {
        radius = (4 * (hLabel + borderWidth) + sqrt(4 * pow(hLabel + borderWidth, 2) + 3 * pow(wScale, 2))) / 3 - hLabel - 2 * borderWidth;
        center = QPointF(0.5 * wWidget, hWidget - borderWidth);
    } else {
        center = QPointF(0.5 * wWidget, radius + hLabel + borderWidth);
    }

    double angleSpan = - 360 / acos(-1) * asin(wScale / (2 * radius));
    ...
    if(majorStep > 5) {
        majorStep = 10 * pow(10, order);
        minorSteps = 5;
    } else if(majorStep > 2) {
        majorStep = 5 * pow(10, order);
        minorSteps = 5;
    } else {
        majorStep = 2 * pow(10, order);
        minorSteps = 4;
    }
    ...
    int offsetCount = (minorSteps - (int)ceil(fmod(minValue, majorStep) / majorStep * minorSteps)) % minorSteps;
    ...
    QRect position;
    Qt::Alignment align;
    for(int i = (int)ceil(minValue / majorStep); i <= (int)(maxValue / majorStep); i++) {
        double temp = acos(-1) / 180 * ((majorStep * i - minValue) / valueSpan * angleSpan + angleStart);
        if(vertical) {
            align = Qt::AlignLeft | Qt::AlignVCenter;
            position = QRect(width - center.y() + radius * sin(temp), 0, width, height + 2 * radius * cos(temp));
        } else {
            align = Qt::AlignHCenter | Qt::AlignBottom;
            position = QRect(0, 0, 2 * (center.x() + radius * cos(temp)), center.y() - radius * sin(temp));
        }
