...
from PyQt5.QtChart import QChartView, QChart, QLineSeries, QLegend,\
    QCategoryAxis, QCandlestickSeries, QCandlestickSet, QStackedBarSeries, QBarSet
from PyQt5.QtCore import Qt, QPointF, QRectF, QPoint, pyqtSignal, QRectF, QMargins
from PyQt5.QtGui import QPainter, QPen, QColor
from PyQt5.QtWidgets import QApplication, QGraphicsLineItem, QWidget, \
    QHBoxLayout, QLabel, QVBoxLayout, QGridLayout, QGraphicsProxyWidget, QMenu, QAction


def generatorTickData():
    """
    生成蜡烛图的数据
...
class CandlestickChartView(QChartView):
    ...
    def __init__(self, *args, **kwargs):
        super(CandlestickChartView, self).__init__(*args, **kwargs)
        self.setRenderHint(QPainter.Antialiasing)  # 抗锯齿
        self.stocks = generatorTickData()
    ...
    def mouseMoveEvent(self, event):
        super(CandlestickChartView, self).mouseMoveEvent(event)
        pos = event.pos()
    ...
    def resizeEvent(self, event):
        super(CandlestickChartView, self).resizeEvent(event)
        # y 轴最高点坐标
        self.point_y_max = self._chart.mapToPosition(QPointF(self.min_x, self.max_y))
        # x 轴最高点坐标
        self.point_x_max = self._chart.mapToPosition(QPointF(self.max_x, self.min_y))
    ...
    def initChart(self):
        ...
        # self._chart.setMargins(QMargins(20, 10, 10, 10))
...
class MainView(QWidget):
    def __init__(self):
        ...
        self.cand = CandlestickChartView()
        self.vol = VolChartView()
        layout = QVBoxLayout()
        layout.addWidget(self.cand)
        layout.addWidget(self.vol)
        layout.setStretch(5, 5)
        self.setLayout(layout)

        self.cand._chart.plotAreaChanged.connect(self.vol.setPlot)
